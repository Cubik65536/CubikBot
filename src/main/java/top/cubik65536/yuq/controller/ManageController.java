...
import top.cubik65536.yuq.logic.QQGroupLogic;
...
public class ManageController {
	...
	public String addManager(GroupEntity groupEntity, @PathVar(0) String type, Long qqNum) {
		switch (type) {
			...
			case "加超管":
				...
				break;
			case "加MC管理":
				groupEntity.setMCJsonArray(groupEntity.getMCJsonArray().fluentAdd(qqNum.toString()));
				break;
			default:
				return null;
		}
	...
	public String delManager(GroupEntity groupEntity, @PathVar(0) String type, Long qqNum) {
		switch (type) {
			...
			case "删超管":
				...
				break;
			case "删MC管理":
				JSONArray mcAdminList = groupEntity.getMCJsonArray();
				BotUtils.delManager(mcAdminList, qqNum.toString());
				groupEntity.setMCJsonArray(mcAdminList);
				break;
			default:
				return null;
		}
	...
	public static class ManageAdminController {
		...
		@Synonym({"loc监控 {status}", "整点报时 {status}", "自动审核 {status}",
				...
				"撤回通知 {status}", "闪照通知 {status}", "复读 {status}", "生草机 {status}",
				"语音识别 {status}", "进群未发言踢出 {status}", "MCDRRemote {status}"})
		@QMsg(at = true)
		public String onOrOff(GroupEntity groupEntity, boolean status, @PathVar(0) String op) {
			...
			switch (op) {
				case "CubikBot":
					...
					prefix = "\nCubikBot " + this.version + "\n\tPowered by Cubik Network\n感谢您使用CubikBot，在您使用中如果遇到任何问题，可以到 Github（ https://github.com/CuBitStudio/CubikBot ）提出 issue，您也可以添加 Cubik交流群（Mjk4NTEzNDg3）进行交流。\n";
					break;
				case "loc监控":
					groupEntity.setLocMonitor(status);
					break;
				case "整点报时":
				...
				case "复读":
					...
					break;
				case "生草机":
					groupEntity.setGrassSpammer(status);
					break;
				case "语音识别":
					groupEntity.setVoiceIdentify(status);
					break;
				case "进群未发言踢出":
					groupEntity.setKickWithoutSpeaking(status);
					break;
				case "MCDRRemote":
					groupEntity.setMCDRRemote(status);
					break;
				default:
					return null;
			}
			groupService.save(groupEntity);
			if (status) return prefix + op + "开启成功";
			else return prefix + op + "关闭成功";
		}
	}
